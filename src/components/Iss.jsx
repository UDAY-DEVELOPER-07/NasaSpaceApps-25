...
// --- (Keep all your existing data and helper functions: partInfoData, meshNames, getMeshes, ClickableMesh, ISSModel) ---
...
const partInfoData = {
    ...
    return (
      <mesh
        ...
        onPointerOver={(e) => {
          ...
          if (material.emissive?.set) material.emissive.set("#00FFFF"); // Neon blue highlight
        }}
        onPointerOut={(e) => {
          e.stopPropagation();
          setHovered(false);
  ...
  function ISSModel({ setSelectedName }) {
    // You should ensure the path to your gltf is correct, e.g., in the public folder
    ...
    useEffect(() => {
      // Preload the model's textures/materials if necessary, but GLTF handles most of it
    ...
    return (
      <group ref={ref}>
        {/* Bounds and Center keep the model properly sized and centered */}
...
// --- START OF IMPROVED UI COMPONENT ---

export default function ISSViewerUI() {
  const [selectedName, setSelectedName] = useState(null);
  
  // Define premium dark theme colors
  const DARK_BG = "#121212"; // Deep black
  const PANEL_BG = "#1E1E1E"; // Slightly lighter for contrast
  const ACCENT_COLOR = "#14E2F1"; // Bright Cyan/Aqua for premium highlights
  const TEXT_COLOR = "#EAEAEA"; // Near-white text
  ...
  // Common style for the info panels (sleek containers)
  const panelStyle = {
    backgroundColor: PANEL_BG,
    border: `1px solid ${ACCENT_COLOR}30`, // Subtle, semi-transparent border
    borderRadius: "10px",
    padding: "1.5rem",
    boxShadow: `0 4px 12px rgba(0, 0, 0, 0.4), 0 0 10px ${ACCENT_COLOR}15`, // Subtle glow effect
    color: TEXT_COLOR,
    fontFamily: 'sans-serif', // Ensure a modern font is used
  };
  ...
  return (
    ...
    >
      {/* Left panel - 3D model container */}
      <div style={{ flex: 1, ...panelStyle, padding: "0" }}>
        <Canvas 
            camera={{ position: [0, 5, 10], fov: 45 }}
            style={{ borderRadius: "10px" }} // Match container border radius
        >
          {/* Lighting for a more dramatic feel */}
          <ambientLight intensity={0.5} color="#FFFFFF" />
          <directionalLight position={[15, 15, 15]} intensity={1.5} color="#FFFFFF" />
          <directionalLight position={[-15, -15, -15]} intensity={0.5} color="#AAAAAA" />
      ...
      <div
        style={{
          width: "350px", // Slightly wider info panel
          display: "flex",
          flexDirection: "column",
          gap: "2rem",
        }}
      >
        {/* Info Panel: Details of the selected part */}
        <div style={{ flex: 2, ...panelStyle, overflowY: "auto", }}>
          <h2 style={{ color: ACCENT_COLOR, borderBottom: `2px solid ${ACCENT_COLOR}`, paddingBottom: "0.5rem", marginBottom: "1rem", fontSize: "1.5rem" }}>
            {selectedName ? "PART DETAIL" : "ISS OVERVIEW"}
          </h2>
          <h3 style={{ color: TEXT_COLOR, fontSize: "1.25rem", marginBottom: "0.5rem" }}>
            {info.title}
          </h3>
          <p style={{ color: TEXT_COLOR, lineHeight: "1.6" }}>
            {info.description}
          </p>
          {selectedName && (
              <p style={{ marginTop: "1rem", color: ACCENT_COLOR }}>
                **Selected Mesh ID:** {selectedName}
              </p>
          )}
        </div>
        ...
        <div style={{ flex: 1, ...panelStyle }}>
          ...
          <p style={{ marginBottom: "0.5rem" }}>
            <span style={{ color: ACCENT_COLOR, fontWeight: "bold" }}>Name:</span> International Space Station
          </p>
          <p style={{ marginBottom: "0.5rem" }}>
            <span style={{ color: ACCENT_COLOR, fontWeight: "bold" }}>Total Meshes:</span> {meshNames.length}
          </p>
          <p>
            <span style={{ color: ACCENT_COLOR, fontWeight: "bold" }}>Interactivity:</span> Click to View Details
          </p>
        </div>
      </div>
    </div>
